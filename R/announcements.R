...
#'
#' @param course_id The ID of the Canvas course to retrieve announcements from
#' @return A data frame containing the announcements
get_announcements <- function(course_id) {
  ...
  args <- list(only_announcements = "1", per_page = "100")

  # Make the API call
  response <- canvas_query(endpoint, args)

  # Process the response and convert it to a data frame
  announcements <- httr::content(response, "parsed")
  announcements_df <- as.data.frame(announcements)
